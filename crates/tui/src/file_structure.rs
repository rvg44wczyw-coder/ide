//! Pure logic behind the File Structure popup (`F12`, `docs/features/
//! tui-file-structure-and-breadcrumbs.md` §2.1/§3.1) -- ported near-
//! verbatim from `crates/ui/src/file_structure.rs`, which has zero `egui`
//! dependency itself, no `App` dependency here either, same shape as
//! `blame_gutter.rs`/`git_gutter.rs`.
...
/// (`tui-file-structure-and-breadcrumbs.md` §3.1). An empty `query`
/// returns every symbol in its natural declaration order, indented by
...
/// to preserve indentation for a filtered result.
