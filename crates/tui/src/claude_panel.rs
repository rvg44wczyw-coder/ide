//! Shells out to the local `claude` CLI for the integrated assistant chat
//! (`docs/features/tui-claude-panel.md` §2.1). Never blocks the main
//! loop -- each prompt runs on a background thread and reports back over
//! a channel polled once per frame (`poll_claude` in `lib.rs`). Never
//! reads, stores, or logs any Claude credential itself; that's the local
//! `claude` CLI's own responsibility. Distinct from `claude_terminal.rs`'s
//! interactive PTY sessions, which this doesn't touch. Ported from
//! `crates/ui/src/claude_panel.rs`, which has no `egui` dependency and so
//! needed no change beyond this module doc comment.
...
impl ClaudePanel {
    /// `pub(crate)`, not private: `app.rs`'s own tests construct a
    /// `ClaudePanel` with a fake runner before exercising
    /// `handle_claude_chat_key`'s `Enter` path, so `App`'s test suite
    /// never triggers the real `run_claude_cli` (which would actually
    /// shell out to the `claude` CLI -- categorically different from
    /// `cargo_panel.rs`'s tests spawning real, fast, free `cargo`
    /// subcommands).
    pub(crate) fn with_runner(runner: Runner) -> Self {
        Self {
            input: String::new(),
            history: Vec::new(),
            queue: Vec::new(),
    ...
    /// (v1 processes one at a time). Non-blocking: returns immediately.
    /// No-op for a blank/whitespace-only prompt.
...
/// lifetime). `-p` with no following argument reads the prompt from
/// stdin.
///
