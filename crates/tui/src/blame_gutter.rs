...
//! into per-line annotations for the blame lane -- no `ratatui`, no I/O,
//! ported near-verbatim from `crates/ui/src/editor/blame_gutter.rs`
//! (`docs/features/tui-blame.md` §2.1).
...
/// Reserved column width for the blame prefix's text, excluding the
/// trailing separator space -- matches `ide-ui`'s `geometry::
/// BLAME_LANE_CHARS` budget. Fixed regardless of annotation text length
/// once blame is toggled on for a tab (`docs/features/tui-blame.md`
/// §2.1).
pub const BLAME_LANE_CHARS: usize = 18;
/// `BLAME_LANE_CHARS` plus one column for the separator space between
/// the label and the buffer's own text.
...
/// an unterminated Unicode bidi override into the lane label
...
/// (commit author names, emails, summaries, bodies) is untrusted; an
/// unterminated override left in place would make everything painted
/// after it in the same text run render in an attacker-chosen visual
/// order, spoofing what the label actually says. Every call site that
/// paints repository-sourced text into this crate's UI must run it
/// through this first.
...
/// truncation actually happened. Shared by the blame lane's inline label
/// and `GitPanel::commit_detail`'s wrapper so an untrusted repository's
/// arbitrarily long commit message (git has no length cap) never reaches
/// this crate's rendering unbounded.
...
/// `annotations` is sorted ascending by `line` (`annotations_from_blame`'s
/// own construction order) -- binary-searches for the run covering
/// `line`, `None` if `line` falls before the first run or past
/// `MAX_BLAME_LINES`' cutoff (no run ever covers it). Shared by the
/// click-to-open-popup path, the keyboard fallback, and the render code
/// deciding whether a given row gets a label.
pub fn blame_annotation_at(
    annotations: &[BlameAnnotation],
    line: usize,
) -> Option<&BlameAnnotation> {
    let idx = annotations.partition_point(|a| a.line <= line);
    if idx == 0 {
        return None;
    }
    let candidate = &annotations[idx - 1];
    if line < candidate.line + candidate.run_len {
        Some(candidate)
    } else {
        None
    }
...
/// date/time-formatting dependency. `now` is a parameter rather than read
/// internally via `SystemTime::now()` so this stays a pure, directly
/// testable function; the one call site (`ui.rs`'s blame-lane/popup paint
/// code) supplies the real clock.
...
mod tests {
    ...
    #[test]
    fn blame_lane_width_is_chars_plus_one() {
        assert_eq!(BLAME_LANE_WIDTH, BLAME_LANE_CHARS + 1);
    }

    #[test]
    fn blame_annotation_at_finds_the_covering_run() {
        let lines = vec![line(0, "aaa"), line(1, "aaa"), line(2, "bbb")];
        let annotations = annotations_from_blame(&lines);
        assert_eq!(
            blame_annotation_at(&annotations, 0).unwrap().commit_id,
            "aaa"
        );
        assert_eq!(
            blame_annotation_at(&annotations, 1).unwrap().commit_id,
            "aaa"
        );
        assert_eq!(
            blame_annotation_at(&annotations, 2).unwrap().commit_id,
            "bbb"
        );
    }

    #[test]
    fn blame_annotation_at_is_none_past_every_run() {
        let lines = vec![line(0, "aaa")];
        let annotations = annotations_from_blame(&lines);
        assert!(blame_annotation_at(&annotations, 5).is_none());
    }

    #[test]
    fn blame_annotation_at_on_empty_annotations_is_none() {
        assert!(blame_annotation_at(&[], 0).is_none());
    }
}
