...
//! history (`docs/features/fleet-shell.md` §2.4). No `IdeApp`/`egui`
//! dependency, same boundary `command.rs`/`keymap.rs`/`find_bar.rs` hold.
...
impl NavHistory {
    ...
    /// (`docs/features/recent-files.md` §2.1) only read of this list.
    /// Independent of `current`/back-forward position: shows the full
    /// visited history, not "from here backward".
...
mod tests {
    ...
    #[test]
    fn recent_locations_is_empty_for_a_fresh_history() {
        let nav = NavHistory::default();
        assert_eq!(nav.recent_locations().count(), 0);
    }

    #[test]
    fn recent_locations_lists_most_recently_pushed_first() {
        let mut nav = NavHistory::default();
        nav.push(loc("a.rs", 0));
        nav.push(loc("b.rs", 5));
        nav.push(loc("c.rs", 9));

        let got: Vec<NavLocation> = nav.recent_locations().cloned().collect();
        assert_eq!(got, vec![loc("c.rs", 9), loc("b.rs", 5), loc("a.rs", 0)]);
    }

    #[test]
    fn recent_locations_reflects_same_file_coalescing() {
        let mut nav = NavHistory::default();
        nav.push(loc("a.rs", 0));
        nav.push(loc("a.rs", 42));

        let got: Vec<NavLocation> = nav.recent_locations().cloned().collect();
        assert_eq!(got, vec![loc("a.rs", 42)]);
    }

    #[test]
    fn recent_locations_is_independent_of_current_back_forward_position() {
        let mut nav = NavHistory::default();
        nav.push(loc("a.rs", 0));
        nav.push(loc("b.rs", 0));
        nav.go_back();
        ...
        let got: Vec<NavLocation> = nav.recent_locations().cloned().collect();
        assert_eq!(got, vec![loc("b.rs", 0), loc("a.rs", 0)]);
    }
}
