...
//! stdout+stderr line by line as the process runs. Mirrors
//! `claude_panel.rs`'s background-thread + `mpsc` poll-once-per-frame
//! pattern; the difference here is a stream of many lines over time
//! rather than one request/response.
...
pub struct CargoPanel {
    ...
    pub running: Option<CargoCommand>,
    rx: Option<Receiver<StreamEvent>>,
}

impl CargoPanel {
    /// Spawns `cargo <subcommand>` with `current_dir(project_root)` and no
    /// other arguments (doc §3), off the UI thread. No-op if a command is
    /// already in flight — v1 runs at most one at a time, unlike
    /// `ClaudePanel`'s prompt queue (doc §3: queuing builds/tests isn't
    /// useful).
    pub fn run(&mut self, project_root: &Path, command: CargoCommand) {
    ...
    /// Call once per frame. Returns `true` if `output`/`running` changed
    /// (the caller should request a repaint).
...
/// `Command::arg` — never formatted into a shell command string — so shell
/// metacharacters in it (which never occur in practice, since `run` only
/// ever passes a fixed literal from `CargoCommand::subcommand`) can't be
/// interpreted.
